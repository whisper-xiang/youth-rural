...
Page({
  ...
  // 计算总分
  calculateTotal() {
    const total = this.data.scoreItems.reduce(
        ...
        if (res.confirm) {
          ...
          try {
            ...
            this.data.scoreItems.forEach((item) => {
              scoreMap[item.key] = item.value;
            });

            // 构建后端期望的数据格式
            const submitData = {
              scoreInnovation: scoreMap.innovation || 0,
              scorePractice: scoreMap.effect || 0,
              scoreEffect: scoreMap.effect || 0,
              scoreReport: scoreMap.presentation || 0,
              comment: this.data.comment,
            };

            await evaluationApi.submitScore(this.data.id, submitData);
