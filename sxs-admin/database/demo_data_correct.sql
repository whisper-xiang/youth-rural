...
-- 插入项目数据（基于schema.sql的project表结构）
...
('PRJ2026001', '乡村振兴调研计划（草稿）', '乡村振兴', '深入农村基层，调研乡村振兴战略实施情况，了解农村发展现状与需求。', '第一周：实地走访调研，收集一手资料\n第二周：数据整理与分析，撰写调研报告', '完成调研报告一份，提出乡村振兴建议方案', '湖南省长沙市望城区', '2026-07-01', '2026-07-15', 5000.00, 3, '13800138001', 8, '王教授', 1, 'draft', NULL, 0, NOW()),
('PRJ2026002', '红色文化传承实践', '文化传承', '探访革命老区，传承红色基因，开展红色文化教育活动。', '第一周：参观革命遗址，收集历史资料\n第二周：开展红色教育活动，撰写实践报告', '完成红色文化调研报告，制作宣传视频', '江西省井冈山市', '2026-07-10', '2026-07-25', 6000.00, 4, '13800138002', 9, '李教授', 2, 'pending', NULL, 0, NOW() - INTERVAL 2 DAY),
('PRJ2026003', '环保宣传进社区', '环保宣传', '深入社区开展环保知识宣传，提高居民环保意识，推广绿色生活方式。', '第一周：社区调研，了解环保现状\n第二周：开展宣传活动，收集反馈意见', '完成社区环保调研报告，建立环保宣传档案', '广东省深圳市南山区', '2026-08-01', '2026-08-15', 4000.00, 5, '13800138003', 10, '张教授', 3, 'pending', NULL, 0, NOW() - INTERVAL 1 DAY),
('PRJ2026004', '科技支农助振兴', '科技支农', '运用现代科技手段帮助农业生产，推广农业新技术，提高农业生产效率。', '第一周：调研当地农业现状\n第二周：推广农业技术，开展培训活动', '完成科技支农报告，建立技术指导档案', '四川省成都市郫都区', '2026-07-05', '2026-07-20', 8000.00, 6, '13800138004', 11, '陈教授', 1, 'college_approved', NULL, 0, NOW() - INTERVAL 3 DAY),
...
('PRJ6010', '乡村振兴调研实践', '乡村振兴', '深入农村基层，调研乡村振兴战略实施情况，了解农村发展现状与需求。', '第一周：实地走访调研，收集一手资料\n第二周：数据整理与分析，撰写调研报告', '完成调研报告一份，提出乡村振兴建议方案', '湖南省长沙市望城区', '2026-05-01', '2026-05-15', 5000.00, 3, '13800138001', 8, '王教授', 1, 'closed', NULL, 1, NOW() - INTERVAL 30 DAY);

-- 插入项目成员（基于schema.sql的project_member表结构）
INSERT INTO project_member (project_id, user_id, role, responsibility, created_at) VALUES
(1, 3, 'leader', '项目负责人，统筹协调', NOW()),
(1, 12, 'member', '调研员，负责实地调研', NOW()),
(1, 13, 'member', '数据分析员，负责数据处理', NOW()),
(2, 4, 'leader', '项目负责人，统筹协调', NOW()),
(2, 14, 'member', '调研员，负责资料收集', NOW()),
(2, 15, 'member', '宣传员，负责活动组织', NOW()),
(2, 16, 'member', '记录员，负责文档整理', NOW()),
(3, 5, 'leader', '项目负责人，统筹协调', NOW()),
(3, 17, 'member', '调研员，负责社区调研', NOW()),
(3, 18, 'member', '宣传员，负责环保宣传', NOW()),
(4, 6, 'leader', '项目负责人，统筹协调', NOW()),
(4, 19, 'member', '技术员，负责技术推广', NOW()),
(4, 20, 'member', '培训师，负责技术培训', NOW()),
(4, 21, 'member', '调研员，负责农业调研', NOW()),
(4, 22, 'member', '记录员，负责文档整理', NOW()),
(5, 7, 'leader', '项目负责人，统筹协调', NOW()),
(5, 23, 'member', '教师，负责教学工作', NOW()),
(5, 24, 'member', '活动组织员，负责文体活动', NOW()),
(5, 25, 'member', '调研员，负责家访调研', NOW()),
...
-- 插入进度记录（基于schema.sql的progress表结构）
...
(10, '调研方案制定', '制定详细的乡村振兴调研方案，确定调研对象和方法。', '2026-05-03', '学校会议室', 3, NOW() - INTERVAL 35 DAY),
(10, '实地调研执行', '深入望城区5个行政村开展实地调研，收集问卷和访谈资料。', '2026-05-08', '湖南省长沙市望城区', 3, NOW() - INTERVAL 30 DAY),
(10, '数据整理分析', '整理调研数据，进行统计分析，撰写调研报告初稿。', '2026-05-12', '学校实验室', 3, NOW() - INTERVAL 28 DAY),
(10, '报告完善提交', '完善调研报告，形成最终成果，提交相关部门。', '2026-05-14', '学校办公室', 3, NOW() - INTERVAL 25 DAY);

-- 插入成果材料（基于schema.sql的result表结构）
...
(10, '乡村振兴调研报告', 'report', '深入调研望城区乡村振兴战略实施情况的详细报告。', '/uploads/covers/rural_report.jpg', 3, 'published', NOW() - INTERVAL 20 DAY),
(10, '调研数据统计分析', 'data', '乡村振兴调研的原始数据和统计分析结果。', '/uploads/covers/data_analysis.jpg', 3, 'published', NOW() - INTERVAL 18 DAY),
(10, '实践照片集', 'other', '记录乡村振兴调研实践过程中的照片集合。', '/uploads/covers/practice_photos.jpg', 3, 'published', NOW() - INTERVAL 15 DAY);

-- 插入成果附件（基于schema.sql的result_attachment表结构）
...
-- 插入成果图片（基于schema.sql的result_image表结构）
...
-- 插入评优活动（基于schema.sql的evaluation表结构）
...
-- 插入评优评分（基于schema.sql的evaluation_score表结构）
...
-- 插入通知附件（基于schema.sql的notice_attachment表结构）
...
-- 插入系统消息（基于schema.sql的sys_message表结构）
