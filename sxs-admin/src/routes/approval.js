...
// 获取待审批列表
router.get('/list', verifyToken, checkRole('college_admin', 'school_admin'), async (req, res) => {
  try {
    ...
    if (role === 'college_admin') {
      ...
      if (status === 'pending') {
        ...
        whereClauses.push("p.status IN ('college_approved', 'school_approved')");
...
// 审批通过
router.post('/approve/:id', verifyToken, checkRole('college_admin', 'school_admin'), async (req, res) => {
  ...
  try {
    ...
    // 检查项目状态
    const [[project]] = await conn.query('SELECT * FROM project WHERE id = ?', [id]);
    if (!project) {
    ...
    // 更新项目状态
    await conn.query('UPDATE project SET status = ? WHERE id = ?', [newStatus, id]);
    ...
    await conn.query(
      ...
      [project.leader_id, '项目审批通过', `您的项目「${project.title}」已通过${role === 'college_admin' ? '学院' : '学校'}审批`, id]
    );
...
// 审批驳回
router.post('/reject/:id', verifyToken, checkRole('college_admin', 'school_admin'), async (req, res) => {
  const conn = await db.getConnection();
  try {
    await conn.beginTransaction();
    ...
    await conn.query(
      `INSERT INTO sys_message (user_id, title, content, type, related_type, related_id)
       VALUES (?, ?, ?, 'approval', 'project', ?)`,
      [project.leader_id, '项目审批被驳回', `您的项目「${project.title}」审批未通过，原因：${opinion}`, id]
    );
