...
router.get('/list', verifyToken, async (req, res) => {
  try {
    ...
    if (role === 'student') {
      ...
      const [[user]] = await db.query('SELECT college_id FROM sys_user WHERE id = ?', [userId]);
...
router.post('/create', verifyToken, checkRole('student'), async (req, res) => {
  ...
  try {
    ...
    const { title, category, description, targetArea, startDate, endDate, budget, teacherId, members } = req.body;
    const userId = req.user.id;

    // 获取用户学院
    const [[user]] = await conn.query('SELECT college_id FROM sys_user WHERE id = ?', [userId]);

    // 生成项目编号
    const projectNo = `SXS${new Date().getFullYear()}${String(Date.now()).slice(-6)}`;
    ...
    const [result] = await conn.query(
      `INSERT INTO project (project_no, title, category, description, target_area, start_date, end_date, budget, leader_id, teacher_id, college_id, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft')`,
      [projectNo, title, category, description, targetArea, startDate, endDate, budget, userId, teacherId, user.college_id]
    );
...
router.put('/update/:id', verifyToken, async (req, res) => {
  try {
    const { id } = req.params;
    const { title, category, description, targetArea, startDate, endDate, budget, teacherId } = req.body;

    // 检查权限
    const [[project]] = await db.query('SELECT * FROM project WHERE id = ?', [id]);
    if (!project) {
      return error(res, '项目不存在');
    }
    ...
    await db.query(
      `UPDATE project SET title = ?, category = ?, description = ?, target_area = ?, 
       ...
       WHERE id = ?`,
      [title, category, description, targetArea, startDate, endDate, budget, teacherId, id]
    );
...
router.post('/submit/:id', verifyToken, async (req, res) => {
  try {
    ...
    await db.query('UPDATE project SET status = ? WHERE id = ?', ['pending', id]);
