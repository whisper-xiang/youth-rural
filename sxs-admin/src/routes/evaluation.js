...
// 获取评优项目列表（专家视角）
router.get('/list', verifyToken, checkRole('expert', 'school_admin'), async (req, res) => {
  try {
...
// 提交评分
router.post('/score/:projectId', verifyToken, checkRole('expert'), async (req, res) => {
  try {
    const { projectId } = req.params;
    const { scoreInnovation, scorePractice, scoreEffect, scoreReport, comment } = req.body;
    const userId = req.user.id;
    ...
    if (existing) {
      ...
      await db.query(
        ...
        [scoreInnovation, scorePractice, scoreEffect, scoreReport, totalScore, comment, projectId, userId]
      );
    } else {
      // 新增评分
      await db.query(
        `INSERT INTO evaluation_score 
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [ep.evaluation_id, projectId, userId, scoreInnovation, scorePractice, scoreEffect, scoreReport, totalScore, comment]
      );
    }
...
// 获取评优结果排名（管理员）
router.get('/ranking', verifyToken, checkRole('school_admin'), async (req, res) => {
  try {
...
// 设置获奖等级
router.post('/award/:projectId', verifyToken, checkRole('school_admin'), async (req, res) => {
  try {
    const { projectId } = req.params;
    ...
    // 更新项目为优秀项目
    if (awardLevel) {
      await db.query('UPDATE project SET is_excellent = 1 WHERE id = ?', [projectId]);
    }
